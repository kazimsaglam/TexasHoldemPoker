...
public class CardDealerAnimation : MonoBehaviour
{
  ...
  private IEnumerator AnimateCardMovement(GameObject card, Vector3 targetPos)
  {
  ...
  private IEnumerator AnimateFoldCardMovement(GameObject card, Vector3 targetPos)
  {
    Vector3 startPos = card.transform.position;
